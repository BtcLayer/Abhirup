...
def main():
    try:
        ...
        try:
            ...
            raise
        def plot_interactive_drawdown(account_id):
            try:
                account_data = df[df['Account'] == account_id].sort_values('Timestamp IST')
                if len(account_data) == 0:
                    logger.warning(f"No data for account {account_id}")
                    return None
                
                # Ensure we have valid data
                account_data = account_data.copy()  # Avoid SettingWithCopyWarning
                account_data['cumulative_pnl'] = account_data['Closed PnL'].cumsum()
                account_data['running_max'] = account_data['cumulative_pnl'].cummax()
                account_data['drawdown'] = (account_data['running_max'] - account_data['cumulative_pnl']).astype(float)
                
                # Convert timestamps to datetime objects if they aren't already
                if not pd.api.types.is_datetime64_any_dtype(account_data['Timestamp IST']):
                    account_data['Timestamp IST'] = pd.to_datetime(account_data['Timestamp IST'], errors='coerce')
                
                # Drop any rows with NaT (invalid dates)
                account_data = account_data.dropna(subset=['Timestamp IST'])
                
                # Create figure
                plt.figure(figsize=(12, 6))
                ax = plt.gca()
                
                # Plot using numeric indices as x-values to avoid datetime issues
                x_vals = np.arange(len(account_data))
                y_vals = account_data['drawdown'].values
                
                # Main plot with proper date formatting
                ax.fill_between(x_vals, y_vals, color='red', alpha=0.3)
                ax.plot(x_vals, y_vals, color='red', linewidth=1.5)
                
                # Set x-axis labels as formatted dates
                date_labels = [ts.strftime('%Y-%m-%d') for ts in account_data['Timestamp IST']]
                ax.set_xticks(x_vals[::max(1, len(x_vals)//10)])  # Show ~10 labels
                ax.set_xticklabels(date_labels[::max(1, len(date_labels)//10)], rotation=45)
                
                ax.set_title(f'Drawdown Analysis: Account {account_id}', pad=20)
                ax.set_xlabel('Date')
                ax.set_ylabel('Drawdown (USD)')
                ax.grid(True, alpha=0.3)
                plt.tight_layout()
                
                logger.debug(f"Generated drawdown analysis for account {account_id}")
                return plt
            
        
            except Exception as e:
                logger.error(f"Error generating drawdown analysis for account {account_id}: {str(e)}", exc_info=True)
                return None
