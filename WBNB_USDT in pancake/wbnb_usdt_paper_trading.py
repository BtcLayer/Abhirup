...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('papertrading_wbnb.log'),
...
SPREADSHEET_KEY = "11GjL8s7mS_AfAdrFyj6ogV5SYOGeuSgGLTmij9TufmA" 
WORKSHEET_NAME = "Sheet1"

# --- Strategy Parameters ---
PAIR = "BNB/USDT"
FEE_TIER = 0.0005  # Represents 0.05% for WBNB/USDT
...
class PaperTradingBot:
    """
    Paper trades a dynamic concentrated liquidity strategy for WBNB/USDT on PancakeSwap
    using live KuCoin data and logs results to Google Sheets.
    """

    def __init__(self):
        logging.info("Initializing Paper Trading Bot for WBNB/USDT...")
    ...
    def _init_exchange(self):
        """Initializes a connection to the KuCoin exchange to avoid restrictions."""
        logging.info("Connecting to KuCoin (Public API)...")
        try:
            exchange = ccxt.kucoin({'enableRateLimit': True})
            exchange.load_markets()
            logging.info(" Successfully connected to KuCoin.")
            return exchange
    ...
    def _get_market_data(self):
        """Fetches the latest price and 1-minute volume from the exchange."""
        try:
    ...
    def _estimate_fees(self, position_value, volume_1m, current_price):
        ...
        our_share_of_activity = position_value / (volume_usd_1m * 1000)
        estimated_fees = (volume_usd_1m * FEE_TIER) * our_share_of_activity * FEE_ESTIMATE_SCALAR
        return min(estimated_fees, position_value * 0.0001)
    ...
    def _update_google_sheet(self, status, price, pos_value, il, fees, pnl, alert):
        ...
        try:
            ...
            row_data = [
                ...
                f"{il:.4f}%", f"${fees:.4f}", f"${pnl:.2f}", alert
            ]
            self.worksheet.append_row(row_data, value_input_option='USER_ENTERED')
        except Exception as e:
            logging.error(f" Could not update Google Sheet: {e}")
